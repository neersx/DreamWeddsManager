...
namespace SmartAdmin.WebUI.EndPoints
{
    ...
    public class UsersEndpoint : ControllerBase
    {
        ...
        private readonly UserManager<BlazorHeroUser> _manager;
        private readonly SmartSettings _settings;

        public UsersEndpoint(BlazorHeroContext context, UserManager<BlazorHeroUser> manager, SmartSettings settings)
        {
            _context = context;
            _manager = manager;
            _settings = settings;
        ...
        public async Task<ActionResult<IEnumerable<BlazorHeroUser>>> Get()
        {
        ...
        [HttpGet("{id}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<ActionResult<BlazorHeroUser>> Get([FromRoute] string id) => Ok(await _manager.FindByIdAsync(id));

        [HttpPost]
        [ProducesResponseType(StatusCodes.Status201Created)]
        public async Task<IActionResult> Create([FromForm] BlazorHeroUser model)
        {
        ...
        public async Task<IActionResult> Update([FromForm] BlazorHeroUser model)
        {
        ...
        [HttpDelete]
        [ProducesResponseType(StatusCodes.Status204NoContent)]
        public async Task<IActionResult> Delete([FromForm] BlazorHeroUser model)
        {
            ...
            var result = await _context.DeleteAsync<BlazorHeroUser>(model.Id);
